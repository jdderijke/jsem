...
from program.DB_Routines import delete_values_in_database
from common_utils import get_newest_file, normalize_data
from Config import HP_POWER, HP_USAGE
...
from Common_Enums import Aggregation, DatabaseGrouping
...
def predict_heatingpower(store_in_db=False, **kwargs):
	...
	try:
		...
		startdate = datetime.now().replace(minute=0, second=0, microsecond=0) - relativedelta(
			hours=frcst_tempcorr_backlook)
		...
		corr_df['frcst_temp'] = corr_df['frcst_temp'].shift(
			frcst_timeshift)  # Move the forecast x hours earlier (-) or later (+)
		...
		data_df['frcst_temp'] = data_df['frcst_temp'].shift(
			frcst_timeshift)  # Move the forecast x hours earlier (-) or later (+)
		...
		Logger.info(
			f'---timeshift ({frcst_timeshift}) and temp correction ({correction}) applied over future buitentemp_act')
		...
		results_df['frcst_power'] = data_df.apply(
			lambda x: model_inference(x, interpreter, input_details[0]['index'], output_details[0]['index']), axis=1)
...
def optimizer(data_df: pd.DataFrame, pph: int):
	...
	hp_planning = get_value_from_database(dpID=hp_plan)
	...
	Logger.info(
		f'---The length of the HP_plan will be {len(data_df)} hours, with a switching pph if {round(1 / pph, 2)} hours')
	...
	try:
		...
		# the plan includes the CURRENT period, in order to avoid nervous HP switching... skip the current period
		# calculate buffer capacity at the end of the current period, because we are NOT changing the run state of the HP
		prev_buf_cap = buf_init - data_df.at[0, 'frcst_power'] - buf_loss + hp_planning * hp_power
		# now throw away any row that has a timestamp before now
		data_df = data_df[data_df['timestamp'] > datetime.timestamp(datetime.now())]
		...
		# iterate through the rest of all periods
			...
			if data_df.at[period, 'buf_cap'] < buf_min:
				...
				for index_min in cheapest:
					...
					if data_df.at[first_possible + index_min, 'hp_runs'] == 0:
						...
						data_df.at[first_possible + index_min, 'hp_cost'] = (data_df.at[
																				 first_possible + index_min, 'epex_info'] + kwh_opslag) * hp_usage
						break
...
def make_hp_plan(power_forecast: pd.DataFrame = None, pph=4, store_in_db: bool = False):
	...
	if store_in_db:
		...
		first_ts = int(plan_df.at[0, 'timestamp'])
		...
		# First clean the DB from earlier optimizer plans
		delete_values_in_database(hp_plan, first_ts)
		# then store the new plan
		store_df_in_database(store_df[['table', 'datapointID', 'timestamp', 'value']])
		...
		# First clean the DB from earlier optimizer plans
		delete_values_in_database(frcst_buffer_energie, first_ts)
		# then store the new plan
		store_df_in_database(store_df[['table', 'datapointID', 'timestamp', 'value']])
