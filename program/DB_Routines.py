...
from Config import DBFILE, DB_RETRIES, DB_WAITBETWEENRETRIES, Max_Chart_Points, DB_alivetime, DB_looptime
from Config import USE_REMOTE_DB_READ, USE_REMOTE_DB_WRITE
from JSEM_Commons import cursor_to_dict, get_begin_of_week
# from interfaces import BaseInterface
from JSEM_Commons import get_type, IsNot_NOE, Is_NOE, Calculate_Period
...
class DBstore_engine(object):
		...
		while self.keeprunning:
			try:
				...
				while self.DBstore_Q:
					...
					# remove any null characters from the query...sqlite3 doesn't like them
					...
					while True:
						try:
							...
							Logger.error(f"This query FAILED..retry: {query}")
	...
	def stop(self):
		...
		self.keeprunning = False
		self.thrd.join()
		Logger.info(f"{self.name}--DBstore_engine queue handled and stopped.")
		self.thrd = None
...
def store_value_in_database(dpID_timestamp_values=[]):
	...
	try:
		...
		store_direct = (Common_Data.DB_STORE is None) or (Common_Data.DB_STORE.thrd is None) or (
					Common_Data.DB_STORE.keeprunning is False)
...
def delete_values_in_database(dpID, from_ts, till_ts=None):
	"""
	This routine deletes values from the Values table for a specific datapoint
	All values from a timestamp from_ts untill a timestamp till_ts will be deleted
	:param dpID: 		Datapoint ID
	:param from_ts: 	Timestamp to delete values from
	:param till_ts: 	Timestamp to delete values till
	:return:
	"""
	if not dpID: raise ValueError("Illegal or missing argument dpID")
	if not from_ts: from_ts = datetime.timestamp(datetime.now())
	if not till_ts:
		query = f"DELETE FROM 'Values' WHERE datapointID={dpID} AND timestamp>={from_ts}"
	else:
		query = f"DELETE FROM 'Values' WHERE datapointID={dpID} AND timestamp>={from_ts} AND timestamp<={till_ts}"

	if USE_REMOTE_DB_WRITE:
		data, _ = tcp_sql_query(query=query)
	elif Common_Data.DB_STORE:
		Common_Data.DB_STORE.add_query(query)
	else:
		db_conn = None
		try:
...
def get_min_max_timestamps(dpIDs=[], pathto_local_DBfile=None):
	...
	try:
		if not pathto_local_DBfile:
			dbfile = DBFILE
		else:
			dbfile = pathto_local_DBfile
...
def stripdatetime(selecteddate, dataselection, correction=0):
	...
	if dataselection.name.startswith('_'):
		return selecteddate - relativedelta(seconds=dataselection.value) + relativedelta(
			seconds=correction * dataselection.value)
	...
	# 	return selecteddate.replace(minute=0, second=0, microsecond=0) + relativedelta(hours=correction)
		...
		return selecteddate.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0) + relativedelta(
			years=correction)
	else:
		return None
...
def create_leading_timestamps(startdate, enddate, datagrouping):
	...
	while checkdate < untildate:
		...
		if datagrouping == DatabaseGrouping._1min:
			checkdate = checkdate + relativedelta(minutes=1)
		if datagrouping == DatabaseGrouping._10min:
			checkdate = checkdate + relativedelta(minutes=10)
		if datagrouping == DatabaseGrouping._15min:
			checkdate = checkdate + relativedelta(minutes=15)
		if datagrouping == DatabaseGrouping._30min:
			checkdate = checkdate + relativedelta(minutes=30)
...
def get_df_from_database(dpIDs=[], dataselection=DataSelection.Day, datagrouping=DatabaseGrouping.All,
						 aggregation=Aggregation.Not,
						 selected_startdate: datetime = None, selected_enddate: datetime = None,
						 dataselection_date=None, maxrows=None,
	...
	Logger.debug('min_dbts: %s, selected_startdate: %s, dataselection: %s, datagrouping: %s, starttime: %s' %
				 (datetime.fromtimestamp(min_dbts), selected_startdate, dataselection.name, datagrouping.name,
				  starttime))
		...
		for dpID in dpIDs:
			...
			if datagrouping != DatabaseGrouping.All:
				if datagrouping == DatabaseGrouping.Year:
					dp_df['group'] = [
						int(datetime.fromtimestamp(x).replace(month=1, day=1, hour=0, minute=0, second=0).timestamp())
						for x in dp_df['timestamp'].values]
				elif datagrouping == DatabaseGrouping.Month:
					dp_df['group'] = [
						int(datetime.fromtimestamp(x).replace(day=1, hour=0, minute=0, second=0).timestamp()) for x in
						dp_df['timestamp'].values]
				elif datagrouping == DatabaseGrouping.Week:
					dp_df['group'] = [int(get_begin_of_week(datetime.fromtimestamp(x)).timestamp()) for x in
									  dp_df['timestamp'].values]
				elif datagrouping == DatabaseGrouping.Day:
					dp_df['group'] = [int(datetime.fromtimestamp(x).replace(hour=0, minute=0, second=0).timestamp()) for
									  x in dp_df['timestamp'].values]
				else:
...
def get_values_from_database(datapoint, maxrows=Max_Chart_Points, data_selection=DataSelection.All,
							 start=datetime.now()):
	...
	try:
		...
		if data_selection == DataSelection.All:
			...
			query = "SELECT * FROM (SELECT value,timestamp FROM 'Values' WHERE datapointID = %s ORDER BY timestamp DESC) ORDER BY timestamp ASC" % (
				datapoint.ID)
		else:
			dt_now = start if start is not None else datetime.now()
			starttimestamp, endtimestamp = Calculate_Period(data_selection=data_selection,
															re_timestamp=int(datetime.timestamp(dt_now)))
			filterstr = " AND timestamp >= %s AND timestamp <= %s" % (str(starttimestamp), str(endtimestamp))
			query = "SELECT * FROM (SELECT value,timestamp FROM 'Values' WHERE datapointID = %s %s ORDER BY timestamp DESC) ORDER BY timestamp ASC" % (
				datapoint.ID, filterstr)
...
# 			query = (
# 						"SELECT * FROM 'Values' WHERE datapointID=%s AND timestamp>=%s AND timestamp <%s order by timestamp %s %s" %
...
def load_protocol(targetobject=None, *args, **kwargs):
	...
	:return:
	"""
	from DataPoint import Protocol
	...
	try:
		...
		if "ID" in kwargs:
			query = "SELECT * FROM Protocol where ID=%s" % (str(kwargs["ID"]))
		elif "name" in kwargs:
			query = "SELECT * FROM Protocol where name='%s'" % (str(kwargs["name"]))
		else:
...
def populate_interface(interf=None, name='', ID=None, **kwargs):
	...
	try:
		...
		if ID:
			query = f"SELECT * FROM Interface where ID={ID}"
		elif name:
			query = f"SELECT * FROM Interface where name='{name}'"
		else:
			raise ValueError("No valid interface ID or name argument passed to routine..")
...
def get_pollmessages_from_database(interfaceID):
	...
	:return:
	"""
	from DataPoint import Pollmessage
	...
	CONN = sqlite3.connect(DBFILE, uri=True)
	try:
		query = "SELECT ID,name,searchkey,poll_interval FROM Datapoints WHERE interfaceID=%s AND enabled=1 AND poll=1 AND poll_interval IS NOT NULL AND poll_interval != ''" % (
			interfaceID)
		# print (query)
		data = CONN.execute(query)
...
def load_all_categories():
	...
	'''
	from DataPoint import Category
...
def get_displaysorted_datapoint_names(categoryID):
	...
	try:
		...
		query = "select sub_cat, name from Datapoints where enabled is not null and display_order is not null and categoryID=%s order by sub_cat ASC, ifnull(display_order,9999) ASC" % (
			categoryID)
...
def load_lastvalues(dpIDs=None):
	'''
	...
	no list of datapointIDs causes the last values and timestamps of ALL datapoints to be retrieved
	'''
	last_values = dict()
