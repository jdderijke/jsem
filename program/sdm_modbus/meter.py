...
class Meter:
	...
	def _decode_value(self, data, length, dtype, vtype):
		try:
			if dtype == registerDataType.INT64:
				return vtype(data.decode_64bit_int())
			elif dtype == registerDataType.UINT64:
				return vtype(data.decode_64bit_uint())
			elif dtype == registerDataType.FLOAT32:
				return vtype(data.decode_32bit_float())
			elif dtype == registerDataType.FLOAT64:
				return vtype(data.decode_64bit_float())
			elif dtype == registerDataType.INT32:
				...
				result = (data.decode_string(size=length*2)).decode()
	...
	def _read_all(self, values, rtype):
		...
		# calculate the minimum address (offset) and the length of the series
